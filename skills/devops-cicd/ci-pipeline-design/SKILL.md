...
description: >
  Use when designing or speeding up a CI pipeline — stage ordering, fail-fast,
  caching, parallelism, what runs on PR vs merge vs nightly. Triggers:
  "CI is too slow", "design the pipeline", "speed up the build",
  "what should run on every PR", "optimize CI".
---
...
- NOT for authoring the workflow syntax itself — see `github-actions-authoring`.
...
## Common pitfalls
- One serial mega-job "because it's simpler." No fail-fast, no parallelism, no per-step timing — the 25-minute monolith everyone tolerates and no one can fix.
- Caching `node_modules` keyed on branch name: stale deps pass on the PR, break on main.
- Running everything on every PR "for safety." Safety that costs 40 minutes gets bypassed with admin merges; the routed design (step 5) is what actually gets run.
- Flakes handled by re-run buttons instead of the false-failure metric. Ten re-runs a day is an unbudgeted engineer.
- Secrets and deploy jobs runnable from fork PRs — pipeline design is also a security boundary; deploy stages trigger only from protected branches.
